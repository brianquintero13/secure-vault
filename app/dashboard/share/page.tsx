...
export default function CreateShareLink() {
    ...
    const [maxUniqueDevices, setMaxUniqueDevices] = useState("");
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input required value={fileName} onChange={(e) => setFileName(e.target.value)} placeholder="e.g. Project Pitch Deck" className="w-full bg-zinc-900 border border-zinc-800 rounded p-2.5 text-white" />
                        </div>
                ...
                <div className="space-y-4 pt-4 border-t border-zinc-900">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="flex items-center gap-4 bg-zinc-900 p-4 rounded border border-zinc-800">
                            ...
                            <div className="flex-1">
                                ...
                                <p className="text-xs text-zinc-500">Forces the client to enter a password before viewing.</p>
                            </div>
                            {requirePassword && (
                                <input required type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Set Password" className="bg-black border border-zinc-700 rounded p-2 text-white text-xs max-w-[120px]" />
                            )}
                        </div>

                        <div className="flex items-center gap-4 bg-zinc-900 p-4 rounded border border-zinc-800">
                            <input type="checkbox" id="lockToFirstDevice" checked={lockToFirstDevice} onChange={(e) => setLockToFirstDevice(e.target.checked)} className="h-5 w-5 accent-emerald-500" />
                            <div className="flex-1">
                                <label htmlFor="lockToFirstDevice" className="font-medium text-zinc-200">Lock to First Device (Anti-Forwarding)</label>
                                <p className="text-xs text-zinc-500">Binds the link strictly to the first network that opens it.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="bg-zinc-900 p-4 rounded border border-zinc-800">
                            <label className="block font-medium text-zinc-200 mb-1">Max Unique Visitors (Devices)</label>
                            <p className="text-xs text-zinc-500 mb-2">Limit how many different people/devices can open this link.</p>
                            <input type="number" value={maxUniqueDevices} onChange={(e) => setMaxUniqueDevices(e.target.value)} placeholder="Leave blank for unlimited" className="w-full bg-black border border-zinc-700 rounded p-2 text-white" />
                        </div>
                        ...
                        <div className="bg-zinc-900 p-4 rounded border border-zinc-800 sm:col-span-2">
                            <label className="block font-medium text-zinc-200 mb-1">Security Watermark Text</label>
                            <p className="text-xs text-zinc-500 mb-2">Adds a dark, transparent background watermark with this text, the viewer's IP, and city.</p>
